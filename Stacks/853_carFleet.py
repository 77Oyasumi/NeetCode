...
class Solution:
    ...
    def carfleet2(self, target: int, pos: List[int], speed: List[int]) -> int:
        fleet_dict = {p:s for p, s in zip(pos, speed)}

        stack = []
        for p in sorted(fleet_dict.keys(), reverse=True):
            stack.append((target - p) / fleet_dict[p])
            if len(stack) >= 2 and stack[-1] <= stack[-2]:
                stack.pop()
        return len(stack)
...
if __name__ == "__main__":
    ...
    target1 = 10; position1 = [3]; speed1 = [3]
    print(solution.carfleet2(target1, position1, speed1))

    target2 = 100; position2 = [0,2,4]; speed2 = [4,2,1]
    print(solution.carfleet2(target2, position2, speed2))